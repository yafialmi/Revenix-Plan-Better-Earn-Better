...
def get_dashboard_summary(
    ...
    if user["role"] == "admin":
        ...
        input_docs = list(
            db.collection("input_data").stream()
        )

        hasil_docs = list(
            db.collection("hasil_perhitungan").stream()
        )

    # ── USER lihat data milik sendiri ──────────────────────────────────────
    else:
    ...
    # Sort by created_at
    perencanaan_list.sort(
        key=lambda x: x.get("created_at", ""),
        reverse=True
    )
    perencanaan_terbaru = perencanaan_list[:3]

    # ── Ambil semua hasil_perhitungan ──────────────────────────────────────
