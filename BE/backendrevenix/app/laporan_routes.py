...
def get_laporan(
    ...
    periode: Optional[int] = Query(
        None,
        description="Filter berdasarkan tahun, contoh: 2025"
    )
):
    """
    ...
    # ── ADMIN lihat semua hasil APPROVED ───────────────────────────────────
    if user["role"] == "admin":
    ...
    # ── USER lihat hasil milik sendiri (hanya approved) ────────────────────
    else:
    ...
    for data in detail_laporan:
        ...
        total_target_revenue += parameter.get(
            "target_revenue",
            0
        )
        total_estimasi_laba += forecast.get(
            "cash_flow",
            0
        )
...
def get_laporan_detail(
    ...
    if data["user_id"] != user["uid"] and user["role"] != "admin":
        raise HTTPException(
            status_code=403,
            detail="Anda tidak punya akses ke data ini"
        )
